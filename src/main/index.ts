...
app.whenReady().then(() => {
    ipcMain.handle(
        'open-file-dialog',
        async (_, title: string, defaultPath?: string, filters?: Electron.FileFilter[]) => {
            ...
            const path = await dialog
                .showOpenDialog(window, { title, filters, defaultPath, properties: ['openFile'] })
                .then((result) => {
                    if (result.canceled) {
    ...
    ipcMain.handle(
        ...
        async (_, title: string, defaultPath?: string, filters?: Electron.FileFilter[]) => {
            ...
            const path = await dialog
                .showSaveDialog(window, { title, filters, defaultPath })
                .then((result) => {
                    if (result.canceled) {
    ...
    ipcMain.on('watch-file', (event, filename) => {
        ...
        watcher = watch(filename, { recursive: false, delay: 200 }, (evt) => {
            switch (evt) {
                case 'update': {
