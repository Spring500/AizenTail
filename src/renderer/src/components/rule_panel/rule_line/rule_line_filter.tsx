...
export const RuleLine_Filter = function ({
    index,
    rules,
    setRules
}: {
    ...
    const renderReg = () => {
        return (
            <div className="ruleBlock" title="根据输入的正则表达式匹配日志条目">
                <RegexTextField
                    fieldName={`${rule.exclude ? '排除' : '包含'}匹配串`}
                    value={rule.reg}
                    placeholder="输入匹配串"
    ...
    return (
        <RuleLineWarpper
            ...
            menuItems={[
                {
                    key: 'exclude',
                    name: () => (rule.exclude ? '包含匹配串' : '排除匹配串'),
                    callback: toggleExclude
                }
            ]}
        >
            {renderReg()}
            <div className="fixedRuleBlock">
                <button
                    className={rule.exclude ? 'ruleButton activatedButton' : 'ruleButton'}
                    onClick={toggleExclude}
                    title="满足该条匹配规则的日志将被筛除还是保留"
                >
                    反向筛选
                </button>
            </div>
